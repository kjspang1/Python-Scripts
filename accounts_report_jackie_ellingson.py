# -*- coding: utf-8 -*-
"""
Created on Tue Nov 19 13:43:43 2019
...
team_ids = accounts.acell('G37').value
team_roles = accounts.acell('H37').value
...
support_names = Convert("moger,ellingson")
while len(support_names) > 0:
    support_name = support_names.pop(0)
    support_query = '''select distinct concat("Support", " - ", first_name) name 
from fos_user where lower(last_name) like '%{}%'
union all
...
join client cl
on cl.id = am.client_id
where am.user_id in ('730')
and cl.is_active = 1
and (sysdate() between start_time and end_time
or start_time between sysdate() and sysdate() + interval 7 day)
and li.account_managers is not null
and li.account_managers like '%{}%'
union all
...
from (select client_id from account_manager 
where client_id in (select id from client where is_active = 1)
and client_id in (select client_id from account_manager where user_id in ('730'))
group by client_id having count(client_id) = '1'
union all
select client_id from (select client_id, count(user_id) count, 
sum(case when is_manager = 1 then 1 else 0 end) as manager from account_manager 
...
on cl.id = am.client_id
where am.user_id in ('730')
and client_id in (select client_id from 
line_item where account_managers like '%{}%')
and cl.is_active = 1) a,
(Select count(distinct li.client_id) active_accounts 
from line_item li
join account_manager am
...
where client_id in (Select distinct client_id from account_manager where user_id in ('730'))
and sysdate() between start_time and date_add(end_time, interval 6 day)
and client_id in (select id from client where type = 'agency')
and account_managers like '%{}%')a)b
union all
...
join client_setting cs 
on  cs.id = cl.clientSetting_id
where li.client_id in (Select distinct client_id from account_manager where user_id in ('730'))
and sysdate() between li.start_time and date_add(li.end_time, interval 6 day)
and li.client_id in (select id from client where type = 'publisher')
and li.account_managers like '%{}%')a)b
union all
...
and sysdate() between start_time and date_add(end_time, interval 6 day)
and client_id in (select id from client where type = 'agency')
and account_managers like '%{}%')a union all
select round(total_active_revenue/7,2) as total_active_revenue from(select case
when li.days_left between 0 and 7 then sum(((li.goal_impressions/1000 *cs.dpm_cpm)/datediff(li.end_time, li.start_time))*days_left)
else  sum(((li.goal_impressions/1000 *cs.dpm_cpm)/datediff(li.end_time, li.start_time))*7)
end as total_active_revenue from line_item li join client cl
...
where user_id in ('730'))
and start_date between sysdate() and sysdate() + interval 90 day
and client_id in (select client_id from line_item where account_managers like '%{}%')
and deal_stage=4
union all
...
and start_date between sysdate() and sysdate() + interval 90 day
and client_id in (select client_id from line_item where account_managers like '%{}%')
    and deal_stage=4) a'''.format(support_name,support_name,support_name,support_name,support_name,support_name,support_name,support_name,support_name,support_name,unsupported,support_name, 
    supported,support_name,support_name,support_name,support_name,support_name,support_name,support_name,support_name,support_name,support_name)
    c.execute(support_query)
    result = [item[0] for item in c.fetchall()]
    query_result.append(result)
...
wb1.save("D://Users//kevinspang//Documents//Excel//Accounts Reports//jackie ellingson//Accounts_Report_Jackie_Ellingson_{}.xlsx".format(today_save))
wb1.close()
wb2.close()
app.kill()